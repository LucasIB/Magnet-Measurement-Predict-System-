        ...
        try:
            for i in range(len(self.dados.files)):
                ...
                if self.trainpar[4].isChecked():
                    ...
                    values.append(self.roll_mean)
##                if self.trainpar[2].isChecked():
##                    self.desl_x = self.dados.Data[i].deslo_x                                  #Displacement X [um]
##                    values.append(self.desl_x)
##                if self.trainpar[5].isChecked():
##                    self.desl_y = self.dados.Data[i].deslo_y                                  #Displacement Y [um]
##                    values.append(self.desl_y)
                self.enter.append(values)
                targets = self.targets()
                values = []
                           
##            self.entrada = np.asarray(enter, dtype=float).reshape((len(enter),len(enter[0])))
##            self.saida = np.asarray(targets, dtype=float).reshape((len(targets), len(targets[0])))
            self.saida = targets
    ...
    def targets(self):
        _index_tar = self.ui.cb_output.currentIndex()
        if _index_tar == 0:
